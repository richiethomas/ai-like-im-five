...
6. CLICHÉS — vague filler and LLM clichés ("honest", "genuine", "game-changer", "rides on", "shines for", "land" as a verb). Also flag grandiose totalizing frames ("X is the entire problem/point/story of Y", "this is what it's all about") and metaphor flourishes that state no mechanism ("the ways the gap opens up"): rewrite as a direct statement of the underlying fact. Also flag figurative or non-literal phrasing where a literal word is available: metaphorical verbs like "unpack", "dive into", "dig into", "peel back", "walk through", "under the hood", "tease apart", and similar. The fix is the literal statement ("unpack it" -> "explain it in detail", "dive into" -> "cover in detail"). ANY use of an em-dash (—) is a violation: report every occurrence, severity 3, with a fix that rewrites the sentence using a period, comma, colon, or parentheses.
